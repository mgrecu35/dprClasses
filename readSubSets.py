...
def readZeroDeg(fname,nr1,nr2):
    ...
    return zeroDeg,sfcRain,binSf,zFactKu,binClut,piaSRT,relFlag,precipType,zFactKa,stormT,sfcType
...
for f in fs[:]:
    nr1=12
    nr2=37
    zeroDeg,sfcRain,binSf,zFactKu,binClut,piaSRT,relFlag,pType,zFactKa,stormT,sfcType=readZeroDeg(f,nr1,nr2)
    #stop
    a=nonzero(sfcRain>0)
    b=nonzero(zeroDeg[a]>0)
    for i1,j1 in zip(a[0][b],a[1][b]):
        if binSf[i1,j1]>167 and pType[i1,j1]==2:
            if binClut[i1,j1]-zeroDeg[i1,j1]<20:
                continue
            else:
                iprof+=1
                ...
                zKu12=zFactKu[i1,j1,zeroDeg[i1,j1]:zeroDeg[i1,j1]+20]
                zKu11=zFactKu[i1,j1,zeroDeg[i1,j1]-60:zeroDeg[i1,j1]]
                if zeroDeg[i1,j1]+20>binClut[i1,j1]:
                    print("error")
                    stop
                z1prof1d=list(zKu11)
                ...
                binSf[i1,j1],zeroDeg[i1,j1],binClut[i1,j1],piaSRT[i1,j1],sfcRain[i1,j1],stormT[i1,j1],\
...
z1L=array(z1L)
import pickle
pickle.dump([z1L,zKuL,zKaL,addInfoL],open('cvProfs.pklz','wb'))
